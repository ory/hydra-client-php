...
**createdAt** | [**\DateTime**](\DateTime.md) | CreatedAt returns the timestamp of the client&#39;s creation. Format: date-time Format: date-time Format: date-time Format: date-time | [optional] 
...
**postLogoutRedirectUris** | **string[]** | Array of URLs supplied by the RP to which it MAY request that the End-User&#39;s User Agent be redirected using the post_logout_redirect_uri parameter after a logout has been performed. | [optional] 
...
**updatedAt** | [**\DateTime**](\DateTime.md) | UpdatedAt returns the timestamp of the last update. Format: date-time Format: date-time Format: date-time Format: date-time | [optional] 
